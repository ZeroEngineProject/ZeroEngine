...
public:
  ...
  /// Operation to perform on stencil value if stencil test passes but depth test fails, for triangle back faces if in
  /// separate mode.
  DeclareByteEnumGetSet(StencilOp::Enum, DepthFailOpBackFace);
  /// Operation to perform on stencil value if both stencil and depth tests pass, for triangle back faces if in separate
  /// mode.
  ...
  /// Value that will be used to compare against the stencil buffer for all pixels, for triangle back faces if in
  /// separate mode.
