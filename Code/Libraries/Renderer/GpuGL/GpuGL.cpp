...
struct Buffer
{
  ...
  GLuint gl_handle;
  BitField<BufferFlags::Enum> flags;
  u64 size;
};
...
struct Program
{
  ...
  BitField<StateFlags::Enum> state = StateFlags::None;
#ifdef ZeroDebug
  // StaticString<64> name;
  String name;
#endif
...
struct GL
{
  ...
  BitField<StateFlags::Enum> last_state = StateFlags::None;
...
struct FormatDesc
{
  ...
  static FormatDesc get(GLenum format)
  {
    ...
    case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
      return get(TextureFormat::BC1);
    case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT:
      return get(TextureFormat::BC2);
    case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
      return get(TextureFormat::BC3);
    case GL_COMPRESSED_RED_RGTC1:
      return get(TextureFormat::BC4);
    case GL_COMPRESSED_RG_RGTC2:
      return get(TextureFormat::BC5);
    case GL_R16:
      return get(TextureFormat::R16);
    case GL_R8:
      return get(TextureFormat::R8);
    case GL_RG8:
      return get(TextureFormat::RG8);
    case GL_SRGB8_ALPHA8:
      return get(TextureFormat::SRGBA);
    case GL_RGBA8:
      return get(TextureFormat::RGBA8);
    case GL_RGBA16:
      return get(TextureFormat::RGBA16);
    case GL_RGBA16F:
      return get(TextureFormat::RGBA16F);
    case GL_R11F_G11F_B10F:
      return get(TextureFormat::R11G11B10F);
    case GL_RGBA32F:
      return get(TextureFormat::RGBA32F);
    case GL_RG32F:
      return get(TextureFormat::RG32F);
    case GL_RG16:
      return get(TextureFormat::RG16);
    case GL_RG16F:
      return get(TextureFormat::RG16F);
    case GL_RGB32F:
      return get(TextureFormat::RGB32F);

    case GL_DEPTH_COMPONENT32:
      return get(TextureFormat::D32);
    case GL_DEPTH24_STENCIL8:
      return get(TextureFormat::D24S8);
    default:
      ASSERT(false);
      return {};
    }
  }
  static FormatDesc get(TextureFormat::Enum format)
  {
    switch (format)
    {
    case TextureFormat::BC1:
      return {true, false, 8, GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT};
    case TextureFormat::BC2:
      return {true, false, 16, GL_COMPRESSED_RGBA_S3TC_DXT3_EXT, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT};
    case TextureFormat::BC3:
      return {true, false, 16, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT};
    case TextureFormat::BC4:
      return {true, false, 8, GL_COMPRESSED_RED_RGTC1, GL_ZERO};
    case TextureFormat::BC5:
      return {true, false, 16, GL_COMPRESSED_RG_RGTC2, GL_ZERO};
    case TextureFormat::R16:
      return {false, false, 2, GL_R16, GL_ZERO, GL_RED, GL_UNSIGNED_SHORT};
    case TextureFormat::RG16:
      return {false, false, 4, GL_RG16, GL_ZERO, GL_RG, GL_UNSIGNED_SHORT};
    case TextureFormat::R8:
      return {false, false, 1, GL_R8, GL_ZERO, GL_RED, GL_UNSIGNED_BYTE};
    case TextureFormat::RG8:
      return {false, false, 2, GL_RG8, GL_ZERO, GL_RG, GL_UNSIGNED_BYTE};
    case TextureFormat::BGRA8:
      return {false, false, 4, GL_RGBA8, GL_SRGB8_ALPHA8, GL_BGRA, GL_UNSIGNED_BYTE};
    case TextureFormat::SRGBA:
      return {false, false, 4, GL_SRGB8_ALPHA8, GL_SRGB8_ALPHA8, GL_RGBA, GL_UNSIGNED_BYTE};
    case TextureFormat::RGBA8:
      return {false, false, 4, GL_RGBA8, GL_SRGB8_ALPHA8, GL_RGBA, GL_UNSIGNED_BYTE};
    case TextureFormat::RGBA16:
      return {false, false, 8, GL_RGBA16, GL_ZERO, GL_RGBA, GL_UNSIGNED_SHORT};
    case TextureFormat::RGBA16F:
      return {false, false, 8, GL_RGBA16F, GL_ZERO, GL_RGBA, GL_HALF_FLOAT};
    case TextureFormat::R11G11B10F:
      return {false, false, 4, GL_R11F_G11F_B10F, GL_ZERO, GL_RGB, GL_UNSIGNED_INT_10F_11F_11F_REV};
    case TextureFormat::RGBA32F:
      return {false, false, 16, GL_RGBA32F, GL_ZERO, GL_RGBA, GL_FLOAT};
    case TextureFormat::RGB32F:
      return {false, false, 12, GL_RGB32F, GL_ZERO, GL_RGB, GL_FLOAT};
    case TextureFormat::RG32F:
      return {false, false, 8, GL_RG32F, GL_ZERO, GL_RG, GL_FLOAT};
    case TextureFormat::RG16F:
      return {false, false, 4, GL_RG16F, GL_ZERO, GL_RG, GL_HALF_FLOAT};
    case TextureFormat::R32F:
      return {false, false, 4, GL_R32F, GL_ZERO, GL_RED, GL_FLOAT};

    case TextureFormat::D32:
      return {false, false, 4, GL_DEPTH_COMPONENT32, GL_ZERO, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT};
    case TextureFormat::D24S8:
      return {false, false, 4, GL_DEPTH24_STENCIL8, GL_ZERO, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT_24_8};
    default:
      ASSERT(false);
      return {};
    }
...
void checkThread()
{
  ASSERT(gl->thread == Thread::GetCurrentThreadId());
}
...
static void logVersion()
{
  ...
  if (version)
  {
    ZPrint("OpenGL version: ", version);
    ZPrint("OpenGL vendor: ", vendor);
    ZPrint("OpenGL renderer: ", renderer);
  }
}
...
static bool load_gl(void* platform_handle, BitField<InitFlags::Enum> init_flags)
{
#ifdef _WIN32
  const bool vsync = init_flags.IsSet(InitFlags::VSYNC);
  ...
  PFNWGLCREATECONTEXTATTRIBSARBPROC wglCreateContextAttribsARB =
      (PFNWGLCREATECONTEXTATTRIBSARBPROC)getGLFunc("wglCreateContextAttribsARB");
...
#  define WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB 0x00000002
  ...
  const int32_t contextAttrs[] = {WGL_CONTEXT_MAJOR_VERSION_ARB,
                                  4,
...
#  endif
  ...
  else
  {
    ...
    Error("wglCreateContextAttribsARB failed, GetLastError() = ", (u32)err);
  ...
  ExternalLibrary gl_dll;
  Status status;
  gl_dll.Load(status, "opengl32.dll");
  gl_dll.mUnloadOnDestruction = false;

#  define GPU_GL_IMPORT(prototype, name)                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
      name = (prototype)getGLFunc(#name);                                                                              \
      if (!name && gl_dll.IsValid())                                                                                   \
      {                                                                                                                \
        name = (prototype)gl_dll.GetFunctionByName(#name);                                                            \
        if (!name)                                                                                                     \
        {                                                                                                              \
          Error("Failed to load GL function " #name ".");                                                              \
          return false;                                                                                                \
...
u32 getSize(TextureFormat::Enum format, u32 w, u32 h)
{
  ...
  if (desc.compressed)
    return sizeDXTC(w, h, desc.internal);
...
static void setVAO(const VertexDecl& decl)
{
  ...
  for (u32 i = 0; i < decl.attributes_count; ++i)
  {
    ...
    case AttributeType::I16:
      gl_attr_type = GL_SHORT;
      break;
    case AttributeType::FLOAT:
      gl_attr_type = GL_FLOAT;
      break;
    case AttributeType::I8:
      gl_attr_type = GL_BYTE;
      break;
    case AttributeType::U8:
      gl_attr_type = GL_UNSIGNED_BYTE;
      break;
    }
...
static void setState(BitField<StateFlags::Enum> state)
{
  GPU_PROFILE();
  checkThread();

  if (state == gl->last_state)
    return;
  gl->last_state = state;
  ...
  if (u64(state & StateFlags::SCISSOR_TEST))
  ...
  auto to_gl = [&](BlendFactors::Enum factor) -> GLenum
  {
    static const GLenum table[] = {GL_ZERO,
                                   GL_ONE,
                                   GL_SRC_COLOR,
  ...
  u16 blend_bits = u16(u64(state.Field) >> 7);

  if (blend_bits)
  {
    const BlendFactors::Enum src_rgb = (BlendFactors::Enum)(blend_bits & 0xf);
    const BlendFactors::Enum dst_rgb = (BlendFactors::Enum)((blend_bits >> 4) & 0xf);
    const BlendFactors::Enum src_a = (BlendFactors::Enum)((blend_bits >> 8) & 0xf);
    const BlendFactors::Enum dst_a = (BlendFactors::Enum)((blend_bits >> 12) & 0xf);
    glEnable(GL_BLEND);
  ...
  glStencilMask(u8(u64(state.Field) >> 23));
  const StencilFuncs::Enum func = (StencilFuncs::Enum)((u64(state.Field) >> 31) & 0xf);
  ...
  {
    const u8 ref = u8(u64(state.Field) >> 35);
    const u8 mask = u8(u64(state.Field) >> 43);
    ...
    case StencilFuncs::EQUAL:
      gl_func = GL_EQUAL;
      break;
    case StencilFuncs::NOT_EQUAL:
      gl_func = GL_NOTEQUAL;
      break;
    case StencilFuncs::DISABLE:
      ASSERT(false);
      break;
    }
    glStencilFunc(gl_func, ref, mask);
    auto toGLOp = [](StencilOps::Enum op)
    {
      const GLenum table[] = {GL_KEEP, GL_ZERO, GL_REPLACE, GL_INCR, GL_INCR_WRAP, GL_DECR, GL_DECR_WRAP, GL_INVERT};
      return table[(int)op];
    };
    const StencilOps::Enum sfail = StencilOps::Enum((u64(state.Field) >> 51) & 0xf);
    const StencilOps::Enum zfail = StencilOps::Enum((u64(state.Field) >> 55) & 0xf);
    const StencilOps::Enum zpass = StencilOps::Enum((u64(state.Field) >> 59) & 0xf);
...
void drawIndirect(DataType::Enum index_type, u32 indirect_buffer_offset)
{
  ...
  glMultiDrawElementsIndirect(
      gl->last_program->primitive_type, type, (const void*)(uintptr)indirect_buffer_offset, 1, 0);
}
...
{
  if (GL_DEBUG_TYPE_PUSH_GROUP == type || type == GL_DEBUG_TYPE_POP_GROUP)
    return;
...
void setCurrentWindow(void* window_handle)
{
  ...
  WindowContext& ctx = [window_handle]() -> WindowContext&
  {
    ...
    {
      if (i.window_handle == window_handle)
        return i;
    }
...
void createBuffer(BufferHandle buffer, BitField<BufferFlags::Enum> flags, size_t size, const void* data)
{
  ...
  if (u64(flags & BufferFlags::IMMUTABLE) == 0)
    gl_flags |= GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT | GL_MAP_READ_BIT;
            ...
            u32 buf_size)
{
  ...
  const bool is_2d = u32(texture->flags & TextureFlags::IS_CUBE) == 0 &&
                     u32(texture->flags & TextureFlags::IS_3D) == 0 && texture->depth == 1;
  const bool is_srgb = u32(texture->flags & TextureFlags::SRGB);

  FixedMemoryStream blob((::byte*)buf, buf_size);
...
static void setSampler(GLuint texture, BitField<TextureFlags::Enum> flags)
{
...
void clear(BitField<ClearFlags::Enum> flags, const float* color, float depth)
{
  ...
  glDisable(GL_SCISSOR_TEST);
  glDisable(GL_BLEND);
  gl->last_state = gl->last_state & ~StateFlags::Enum(0xffFF << 6);
  checkThread();
  GLbitfield gl_flags = 0;
  ...
  {
    glStencilMask(0xff);
    gl->last_state = gl->last_state | StateFlags::Enum(0xff << 22);
    glClearStencil(0);
...
static const char* shaderTypeToString(ShaderType::Enum type)
{
  switch (type)
  {
                   ...
                   const char* name)
{
  ...
  enum
  {
    MAX_SHADERS_PER_PROGRAM = 16
  };

  if (num > MAX_SHADERS_PER_PROGRAM)
  {
    Error("Too many shaders per program in ", name);
    return;
  }
  ...
  for (u32 i = 0; i < num; ++i)
  {
    ...
    case ShaderType::GEOMETRY:
    {
      combined_srcs[src_idx] = "#define ZERO_GEOMETRY_SHADER\n";
      shader_type = GL_GEOMETRY_SHADER;
      break;
    }
    case ShaderType::COMPUTE:
    {
      combined_srcs[src_idx] = "#define ZERO_COMPUTE_SHADER\n";
      shader_type = GL_COMPUTE_SHADER;
      break;
    }
    case ShaderType::FRAGMENT:
    {
      combined_srcs[src_idx] = "#define ZERO_FRAGMENT_SHADER\n";
      shader_type = GL_FRAGMENT_SHADER;
      break;
    }
    case ShaderType::VERTEX:
    {
      combined_srcs[src_idx] = "#define ZERO_VERTEX_SHADER\n";
    ...
    if (compile_status == GL_FALSE)
    {
      ...
      if (log_len > 0)
      {
        ...
        Error(name, " - ", shaderTypeToString(types[i]), ": ", &log_buf[0]);
      }
      else
      {
        Error("Failed to compile shader ", name, " - ", shaderTypeToString(types[i]));
      }
      glDeleteShader(shd);
      return;
    }
  ...
  if (linked == GL_FALSE)
  {
    ...
    if (log_len > 0)
    {
      ...
      Error(name, ": ", &log_buf[0]);
    }
    else
    {
      Error("Failed to link program ", name);
    }
    glDeleteProgram(prg);
    return;
  }
  ...
  case PrimitiveType::TRIANGLES:
    prog->primitive_type = GL_TRIANGLES;
    break;
  case PrimitiveType::TRIANGLE_STRIP:
    prog->primitive_type = GL_TRIANGLE_STRIP;
    break;
  case PrimitiveType::LINES:
    prog->primitive_type = GL_LINES;
    break;
  case PrimitiveType::POINTS:
    prog->primitive_type = GL_POINTS;
    break;
  case PrimitiveType::NONE:
    prog->primitive_type = 0;
    break;
  default:
    ASSERT(0);
    break;
...
void memoryBarrier(BitField<MemoryBarrierType::Enum> type, BufferHandle)
{
  GPU_PROFILE();
  GLbitfield gl = 0;
  if (u32(type & MemoryBarrierType::SSBO))
    gl |= GL_SHADER_STORAGE_BARRIER_BIT;
  if (u32(type & MemoryBarrierType::COMMAND))
...
bool init(void* window_handle, BitField<InitFlags::Enum> init_flags)
{
  GPU_PROFILE();
#ifdef ZeroDebug
  ...
  gl->thread = Thread::GetCurrentThreadId();
  gl->contexts[0].window_handle = window_handle;
#ifdef _WIN32
  gl->contexts[0].device_context = GetDC((HWND)window_handle);
  if (!load_gl(gl->contexts[0].device_context, init_flags))
    return false;
#else
  if (!load_gl(window_handle, init_flags))
    return false;
#endif
  ...
  const ShaderType::Enum default_types[] = {ShaderType::FRAGMENT, ShaderType::VERTEX};
  createProgram(gl->default_program,
                StateFlags::None,
                VertexDecl(PrimitiveType::NONE),
                default_srcs,
                default_types,
                2,
                nullptr,
  ...
  gl->last_state = StateFlags::Enum(1);
...
void copy(TextureHandle dst, TextureHandle src, u32 dst_x, u32 dst_y)
{
  ...
  while ((src->width >> mip) != 0 || (src->height >> mip) != 0)
  {
    ...
    {
      glCopyImageSubData(
          src->gl_handle, src->target, mip, 0, 0, 0, dst->gl_handle, dst->target, mip, dst_x, dst_y, 0, w, h, 6);
    }
    else
    {
      glCopyImageSubData(
          src->gl_handle, src->target, mip, 0, 0, 0, dst->gl_handle, dst->target, mip, dst_x, dst_y, 0, w, h, 1);
    }
    ++mip;
    if (u32(src->flags & TextureFlags::NO_MIPS))
      break;
    if (u32(dst->flags & TextureFlags::NO_MIPS))
      break;
  }
}
...
QueryHandle createQuery(QueryType::Enum)
{
  GPU_PROFILE();
  GLuint q;
  glGenQueries(1, &q);
...
void setFramebuffer(const TextureHandle* attachments, u32 num, TextureHandle ds, BitField<FramebufferFlags::Enum> flags)
{
  GPU_PROFILE();
  checkThread();
