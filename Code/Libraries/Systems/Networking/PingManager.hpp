...
public:
  ...
  /// Specify port 0 to indicate we should send a ping message on all ports within our inclusive host range (configured
  /// with SetHostPortRangeStart/End). Returns true if successful, else false.
  ...
  /// Sets the CancelledCallback. This happens when one ping overwrite another and is canceled before starting the next
  /// one.
  ...
  float mPingInterval; ///< Controls how often the peer sends a host ping message when discovering or refreshing hosts
                       ///< via ping messages.
};
