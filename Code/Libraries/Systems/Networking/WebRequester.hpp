...
public:
  ...
  /// If this value is non-zero, we will automatically cache a file in a temporary location for this many seconds when
  /// the response is OK. Subsequent requests to the same URL will automatically return the last cached request. If the
  /// request fails in any way, then we also return the last cached request even if it's expired. Note that this is
  /// forced caching, and does not query the url to see if a newer version exists. WARNING: This only caches by url, and
  /// does NOT use post data or request headers to differentiate a response. Note that we will only get the
  /// WebResponseComplete event and no headers/partial data if the cache is found. Also note that the cache finding is
  /// not executed on another thread, so mSendEventsOnRequestThread has no effect. Currently can only cache data when
  /// StoreData is set to true. Default is 0 (no caching).
