...
public:
  ...
  /// Updates the host data state from no response to basic hose info (taking into account if it was direct or indirect
  /// info).
...
{
public:
  /// Called in order to prematurely shutdown a host request. Dispatches events, expects that this host request cleans
  /// up anything it created
...
public:
  ...
  /// Its expected users use the pong helper. Then in handle pong they just have to decide how they want to handle the
  /// pong in terms of doing additional host discovery or not. (LAN will call it good, internet will likely want to do
  /// another more direct search)
  ...
  /// A function which is called when the net peer closes. If a net host discovery instance did something, this is where
  /// it can cleans it up.
  ...
  /// Initialized must be called to set callbacks and to listen to events of the net peer. (because net discovery often
  /// causes events, and must handle them itself)
  void Initialize();
  ...
  /// A function which terminates an events propagation if the event was caused by something internal, and we don't
  /// really want the user to see or deal with it. Example: Master server communication and exchanging extra host info.
  ...
  /// Called on net discovery interface in order to dispatch events, and clean up all other information related to the
  /// host request.
