...
public:
  ...
  /// [Client/Server] Creates the complete family tree representing this net object as the ancestor and all of it's net
  /// object children recursively as descendants.
  void InitializeFamilyTree();
  void AddDownFamilyTree(NetObject* ancestor);

  /// Reads identification information (such as IsAbsent, ReplicaId, IsCloned, IsEmplaced, EmplaceContext, and
  /// EmplaceId) from the replica stream.
  ...
  /// Controls whether or not net channels on this net object may nap (perform change detection on longer intervals) if
  /// they haven't changed in a while.
  void SetAllowNapping(bool allowNapping = true);
  bool GetAllowNapping() const;

  /// Controls whether or not the net object will serialize an accurate timestamp value when brought online, or will
  /// instead accept an estimated timestamp value.
  void SetAccurateTimestampOnOnline(bool accurateTimestampOnOnline = false);
  bool GetAccurateTimestampOnOnline() const;

  /// Controls whether or not the net object will serialize an accurate timestamp value when changed (on any net
  /// channel), or will instead accept an estimated timestamp value. (Enabling this will override the corresponding net
  /// channel type setting for all net channels added to this net object)
  void SetAccurateTimestampOnChange(bool accurateTimestampOnChange = false);
  bool GetAccurateTimestampOnChange() const;

  /// Controls whether or not the net object will serialize an accurate timestamp value when taken offline, or will
  /// instead accept an estimated timestamp value.
  ...
  /// Returns true if all net channels on this net object are napping (performing change detection on longer intervals),
  /// else false.
  ...
  /// (Only applies to non-emplaced net objects)
  /// [Client/Server] Returns the family tree ID this net object belongs to (either as an ancestor or descendant), else
  /// 0.
  ...
  /// Returns true if the net object was created as part of a cog initialization (not a level initialization), else
  /// false.
  ...
  /// [Client/Server] Selects the remote net object on the first opposite-role peer found running in another game
  /// session instance on the engine. Will fail if the net object is not online, or not found remotely. Returns true if
  ...
  /// Returns true if our open peer's network role is client and the net object is conceptually owned by a user added by
  /// our local peer, else false.
  bool IsClientAndMine() const;
  /// Returns true if our open peer's network role is client and the net object is not conceptually owned by a user
  /// added by our local peer, else false.
  bool IsClientButNotMine() const;

  /// Returns true if our open peer's network role is server and the net object is conceptually owned by a user added by
  /// our local peer, else false.
  bool IsServerAndMine() const;
  /// Returns true if our open peer's network role is server and the net object is not conceptually owned by a user
  /// added by our local peer, else false.
  bool IsServerButNotMine() const;

  /// Returns true if our open peer's network role is offline and the net object is conceptually owned by a user added
  /// by our local peer, else false.
  bool IsOfflineAndMine() const;
  /// Returns true if our open peer's network role is offline and the net object is not conceptually owned by a user
  /// added by our local peer, else false.
  ...
  /// [Server/Offline] Sets the owning network user on this object and up the tree on each parent recursively (pre-order
  /// traversal).
  void SetNetUserOwnerUp(Cog* cog);
  void SetNetUserOwnerUpById(NetUserId netUserId);

  /// [Server/Offline] Sets the owning network user on this object and down the tree on all children recursively
  /// (pre-order traversal).
  ...
  /// Sets the automatic net channel configuration resource (assigned to net properties unless another channel is
  /// specified).
  void SetAutomaticChannel(NetChannelConfig* netChannelConfig);
  /// Returns the automatic net channel configuration resource (assigned to net properties unless another channel is
  /// specified).
  ...
  // Data
  String mInitLevelResourceIdName; ///< Initialization level resource ID name (if created as part of a level
                                   ///< initialization).
...
// Attachments, Detachments, and Reorders are IGNORED when serializing replicas in their original creation hierarchy
// structure. This is because we use Family Trees which represent the original net object archetype hierarchy order.
// Family Trees are carefully updated to mark destroyed or forgotten net objects as absent. Absent net objects are
// destroyed during their C++ AllObjectsCreated phase. This means replication occurs indirectly, by replicating Family
// Trees according to which of their net objects are still present (not absent). Remotely, after deserializing replicas
// in their original creation hierarchy structure, Attachments, Detachments, and Reorders are then applied. Effectively,
// replication occurs indirectly by transmitting replicas as they were originally created and then updating them via
// their net properties immediately after the fact. Specifically, Attachments, Detachments, and Reorders occur when
// reading ReplicaData inside the C++ AllObjectsCreated phase. Because the C++ AllObjectsCreated phase is already in a
// predetermined order, this hierarchy shifting will not break our deserialization order.

// "External" Attachments (attaching other objects not a part of an internal archetype hierarchy) are performed once
// both the Parent and Child exist. Normally this is not a problem, except in the case of clients receiving a network
// game. It's very possible that parents and children are replicated out of order. Because of our use of Family Trees,
// it's not possible to replicate external attachments in their new hierarchy order. These relations unfortunately can
// become cyclic between Family Trees. So, in the case of external attachments, it is undefined whether or not a parent
// or it's child will be replicated first. (Internal attachments are never a problem) If the Parent is replicated first:
// (Instant) When the Child is replicated, the Child will attach to the Parent inside the Child's "C++
// AllObjectsCreated" phase, while the Child is reading it's ReplicaData. If the Child is replicated first: (Delayed)
// When the Parent is replicated, the Child will attach to the Parent inside the Parent's "C++ AllObjectsCreated" phase,
// *after* the Parent reads it's ReplicaData as a special case. Finally, it is guaranteed that all external attachments
// (Instant or Delayed) will be correctly applied prior to the NetLevelStarted and NetGameStarted Events.
