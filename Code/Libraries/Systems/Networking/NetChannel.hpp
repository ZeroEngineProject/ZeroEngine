...
public:
  ...
  /// Returns true if the net channel is currently napping (performing change detection on longer intervals), else
  /// false.
  ...
  /// The server is still responsible for relaying contained property changes to other clients, but will not replicate
  /// contained property changes back to the authority client. However, the server is also still responsible for other
  /// replication commands (such as object creation/destruction), and these WILL be replicated to the authority client.
...
public:
  ...
  /// Controls whether or not net channels should dispatch NetChannelOutgoingPropertyChange when an outgoing net
  ...
  /// Controls whether or not net channels should dispatch NetChannelIncomingPropertyChange when an incoming net
  ...
  /// Controls whether or not net channels will have their changes immediately broadcast to all relevant, incidental
  /// peers (if any) once received. (Enabling this allows a server to automatically relay client authoritative changes
  /// to other clients, otherwise this must be done manually using NetChannel::ReplicateNow)
  void SetAllowRelay(bool allowRelay = true);
  bool GetAllowRelay() const;

  /// Controls whether or not net channels may nap (perform change detection on longer intervals) if they haven't
  /// changed in a while.
  void SetAllowNapping(bool allowNapping = true);
  bool GetAllowNapping() const;

  /// Controls the frame duration following the last detected change in which net channels are considered actively
  ...
  /// If enabled, all net channel property values are guaranteed to be set immediately before the NetObjectOffline
  /// event. (Cannot be modified at game runtime)
  ...
  /// (Ordered: Changes are released immediately once preceding late changes have been received; forces all changes to
  /// be sent reliably) (Cannot be modified at game runtime)
  void SetTransferMode(TransferMode::Enum transferMode = TransferMode::Ordered);
  TransferMode::Enum GetTransferMode() const;

  /// Controls whether or not the net channel will serialize an accurate timestamp value when changed, or will instead
  /// accept an estimated timestamp value. (This setting may be overridden for net channels belonging to a specific net
  /// object by enabling the corresponding net object setting)
