...
public:
  ...
  // All Fields will be turned into GetterSetters, and all GetterSetters will be directly wrapped with our own
  // GetterSetters Functions will also be wrapped and the return value passed through Currently only types with default
  // constructors will work (the generated wrapper also will have a default constructor) In the future the generated
  // type will receive all the same constructors (and will forward the parameters to the wrapped type upon construction)
