...
public:
  ...
  /// Invokes Seek, and if it fails it will attempt to clamp the position either to 0 or to the end of the stream and
  /// Seek again Check the stream capabilities for Seek before setting the position
  ...
  /// the stream Note: '\r' is not used as a line terminator because that format is out-dated Read line always returns
  /// the line including the '\n' at the end If a line does include a '\n' at the end, it means we reached the end of
  /// the stream (or an error occurred) An empty string will be returned if ReadLine is called again after it already
