...
DeclareEnum9(LinkState,
             ...
             SentNegativeConnectResponse, /// Sent a negative connect response, awaiting their ACK or ((RTT/2) * connect
                                          /// attempt factor) timeout before being disconnected
             ReceivedNegativeConnectResponse, /// Received a negative connect response, waiting ((RTT/2) * connect
                                              /// attempt factor) for probable ACKing before being disconnected

             // LinkStatus::AttemptingConnection
             SentConnectRequest,     /// Sent a connect request, awaiting their connect response or ((RTT/2) * connect
                                     /// attempt factor) timeout before being disconnected
             ReceivedConnectRequest, /// Received an appropriate connect request, they're awaiting our connect response
                                     /// or ((RTT/2) * connect attempt factor) timeout before being disconnected

             // LinkStatus::Connected
             Connected); /// Connected to the remote peer, may send or receive a disconnect notice by request, latency
                         /// (latency limit), or timeout ((RTT/2) * timeout factor)
...
DeclareEnum3(ACKState,
    ...
    DeclareEnum3(TransferMode,
                 ...
                 Ordered);  /// Messages released immediately, in sent order, once preceding late messages have been
                            /// received; forces all messages to be reliable
...
// Sequenced (Reliable)  : 1  4  *X* 'X' '5' Released upon arrival, but discarded if out-of-order, and retransmits lost
// messages Ordered   (Reliable)  : 1 '2' *3*  4  '5' Released upon arrival when prior messages have been received,
// re-orders messages, and retransmits lost messages
// ------------------------------------------------------------
