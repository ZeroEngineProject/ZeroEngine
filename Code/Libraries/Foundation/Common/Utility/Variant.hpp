...
/// Useful for dynamic programming and storing complex user data. Variant utilizes automatically generated native type
/// meta. A variant may contain any native type which has both an accessible copy constructor and destructor. Stored
/// values must be retrieved (using Is/Get/etc.) exactly as the types they were assigned as, ignoring qualifications.
/// Variant cannot implicitly up-cast nor down-cast polymorphic types. Higher or lower types are considered to be
/// completely unrelated. All types are first unqualified (stripped of their top-most reference and cv-qualifiers) on
/// assignment, retrieval, and comparison. Variant is optimized to work best with small types which can be directly
/// constructed within variant's fixed-size local buffer. Larger types, which do not fit within variant's fixed-size
/// local buffer, are constructed on the heap and internally managed by pointer instead. Regardless of whether or not a
/// stored type is large or small, the user does not need to change how they interact with variant. The distinction
/// between large and small types is only made known for performance considerations (large types cause memory allocation
/// on assignment).
...
public:
  ...
  /// (If valueAddress is null, the variant is assigned a default constructed value of the specified native type
  /// instead)
  ...
  /// Parses the string as a value of the specified type T and assigns the parsed value to the variant, else clears the
  /// variant
  template <typename T>
  void ToValue(StringRange range);
  /// Parses the string as a value of the specified native type and assigns the parsed value to the variant, else clears
  /// the variant
  void ToValue(StringRange range, NativeType* nativeType);
  /// Parses the string as a value of the variant's stored type and assigns the parsed value to the variant, else clears
  /// the variant
  void ToValue(StringRange range);
  ...
  /// Returns the stored value's primitive member at the specified index if the variant stored type is T, else asserts
  /// and returns an invalid primitive
  template <typename T,
  ...
  /// (Only defined for arithmetic types)
  /// Returns the stored value's primitive member at the specified index if the variant stored type is T, else
  /// defaultValue
  template <typename T,
            typename UnqualifiedType = typename Decay<T>::Type,
            TF_ENABLE_IF(IsBasicNativeTypeArithmetic<UnqualifiedType>::Value),
  ...
  /// (Only defined for arithmetic types)
  /// Returns a pointer to the stored value's primitive member at the specified index if the variant stored type is T,
  /// else nullptr
  template <typename T,
            typename UnqualifiedType = typename Decay<T>::Type,
            TF_ENABLE_IF(IsBasicNativeTypeArithmetic<UnqualifiedType>::Value),
...
struct ZeroShared MoveWithoutDestructionOperator<Variant>
{
  static inline void MoveWithoutDestruction(Variant* dest, Variant* source)
  {
    ...
    // Source variant should now be empty, which means it should also be zeroed, which means it's safe to skip calling
    // it's destructor (As a sanity check we're going to explicitly verify that the source variant is now both empty and
    // zeroed.
