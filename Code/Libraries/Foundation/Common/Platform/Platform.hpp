...
// For some weird reason it requires casting the char to an 'unsigned char', even though isspace is defined to take an
// int This is well formed C and C++ code, static analysis is complaining about it
#  pragma warning(disable : 6330)

// Static analysis also complains about the use of alloca, which has a well defined meaning
// It recommends the usage of _malloca, a completely non-standard MSVC only function which functions EXACTLY the same in
// Release mode
...
// Static analysis also complains about 'Dereferencing NULL pointer' for every single instance we use in-place new,
// which is simply not correct
#  pragma warning(disable : 6011)

// Comparing a constant to another constant (not caught by the W4 via templates) is considered bad by static analysis,
// even though it completely makes sense and will be optimized out if it is in fact a constant
...
//    if (globalPlayer->IsAlive()) <--- Here it complains that 'globalPlayer' could be null, simply because of the below
//    line
...
// Standard examples (such as setting thread name) from Microsoft's own documentation cause the static analysis to
// complain about __try and __catch (about continuing from the exception possibly causing an infinite loop, which it
// does not)
...
// ZilchDeclareType is used on classes that sometimes derive from a base class with ZilchGetDerivedType() creating a
// discrepancy we can't avoid
...
// When we force inline some functions, gcc has a very complicated way of determining if a function can be inlined or
// not If it decides the function does not get inlined, and the function was actually implemented in the cpp (not the
// header, link time code-gen) then it complains about the function being unused (which it's absolutely not, seems like
// a gcc bug)
